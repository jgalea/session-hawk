#!/usr/bin/env python3
"""
Session Hawk hook client — portable Python implementation.

Drop-in replacement for the Swift SessionHawkHooks binary.
...
Usage (configured in Claude Code settings.json or Codex hooks.json):
    echo '{"hook_event_name":"SessionStart",...}' | python3 session-hawk-hooks.py --source claude
"""
...
def socket_path():
    path = os.environ.get("SESSION_HAWK_SOCKET_PATH") or \
           os.environ.get("VIBE_ISLAND_SOCKET_PATH")
    if path:
        return path
    return f"/tmp/session-hawk-{os.getuid()}.sock"
