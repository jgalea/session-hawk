...
HOOK_SCRIPT="$SCRIPT_DIR/session-hawk-hooks.py"
REMOTE_BIN_DIR=".local/bin"
REMOTE_HOOK_PATH="\$HOME/$REMOTE_BIN_DIR/session-hawk-hooks.py"
...
REMOTE="$1"
LOCAL_UID=$(id -u)
SOCKET_NAME="session-hawk-${LOCAL_UID}.sock"

echo "==> Deploying session-hawk-hooks.py to $REMOTE ..."
ssh "$REMOTE" "mkdir -p ~/$REMOTE_BIN_DIR"
scp "$HOOK_SCRIPT" "$REMOTE:~/$REMOTE_BIN_DIR/session-hawk-hooks.py"
ssh "$REMOTE" "chmod +x ~/$REMOTE_BIN_DIR/session-hawk-hooks.py"
...
# forwarded socket created by the local Session Hawk app.  The heredoc is
# unquoted so that $SOCKET_NAME is expanded.
HOOK_CMD="SESSION_HAWK_SOCKET_PATH=/tmp/$SOCKET_NAME python3 ~/.local/bin/session-hawk-hooks.py --source claude"
