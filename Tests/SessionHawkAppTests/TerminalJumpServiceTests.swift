...
final class TerminalJumpServiceTests: XCTestCase {
    ...
    func testGhosttyJumpIntegrationMatchesFocusedTerminalForLiveSurfaces() throws {
        guard ProcessInfo.processInfo.environment["SESSION_HAWK_RUN_GHOSTTY_JUMP_INTEGRATION"] == "1" else {
            throw XCTSkip("Set SESSION_HAWK_RUN_GHOSTTY_JUMP_INTEGRATION=1 to run live Ghostty jump verification.")
        }
    ...
    func testCursorJumpActivatesRunningAppWithoutWorkspaceReuse() throws {
        ...
        let result = try service.jump(
            to: JumpTarget(
                ...
                workingDirectory: "/tmp/test/open-vibe-island"
            )
        )
