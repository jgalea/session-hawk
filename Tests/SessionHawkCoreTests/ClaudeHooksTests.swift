...
struct ClaudeHooksTests {
    ...
    func claudeHookOutputEncoderEncodesPermissionDecision() throws {
        let output = try ClaudeHookOutputEncoder.standardOutput(
            for: .claudeHookDirective(
                .permissionRequest(
                    .deny(message: "Permission denied in Session Hawk.", interrupt: true)
                )
            )
        )
        ...
        #expect(decision?["message"] as? String == "Permission denied in Session Hawk.")
    ...
    func claudeHookInstallationManagerRoundTripsInstallAndUninstall() throws {
        let rootURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("session-hawk-claude-hooks-\(UUID().uuidString)", isDirectory: true)
        ...
        let managedHooksBinaryURL = rootURL
            ...
            .appendingPathComponent("SessionHawkHooks")
    ...
    func claudeTranscriptDiscoveryRecoversRecentSessions() throws {
        let rootURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("session-hawk-claude-discovery-\(UUID().uuidString)", isDirectory: true)
    ...
    func claudeTranscriptDiscoveryStreamsTranscriptsLargerThanReadChunk() throws {
        ...
        let rootURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("session-hawk-claude-discovery-stream-\(UUID().uuidString)", isDirectory: true)
        let workspaceDirectory = rootURL
            .appendingPathComponent("projects", isDirectory: true)
            .appendingPathComponent("-tmp-demo-repo", isDirectory: true)
    ...
    func claudeTranscriptDiscoveryHandlesTrailingLineWithoutNewline() throws {
        ...
        // split would.
        let rootURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("session-hawk-claude-discovery-trailing-\(UUID().uuidString)", isDirectory: true)
        let workspaceDirectory = rootURL
            .appendingPathComponent("projects", isDirectory: true)
            .appendingPathComponent("-tmp-demo-repo", isDirectory: true)
    ...
    @Test
    func claudeInferTerminalAppPrefersTermProgramOverLeakedGhosttyEnvVars() {
        // Regression: launching one terminal from a Ghostty tab leaks
        // GHOSTTY_RESOURCES_DIR (and friends) into the new shell via
        // macOS GUI app environment inheritance. An env-var-first ordering
        // would tag that shell as Ghostty, causing terminalLocator to query
        // Ghostty's focused tab and stamp a foreign Ghostty pane onto the
        // wrong session's jumpTarget. TERM_PROGRAM is the only signal that
        // doesn't leak this way and must dominate per-app env vars.
