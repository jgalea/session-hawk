...
struct HookSkipConfigurationTests {
    /// Accepts common truthy spellings for the preferred Session Hawk key.
    /// 推荐的新环境变量接受常见真值写法。
    @Test
    func sessionHawkSkipHooksAcceptsTruthyValues() {
        for value in ["1", "true", "TRUE", "yes", "on", " 1 "] {
            #expect(HookSkipConfiguration.shouldSkipHooks(environment: [
                HookSkipConfiguration.sessionHawkSkipKey: value,
            ]))
        }
    }
