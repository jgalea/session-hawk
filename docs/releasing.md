...
How to cut a new GitHub release for Session Hawk.
...
2. **Build & package**:
   ...
   SESSION_HAWK_EDDSA_PUBLIC_KEY="<your-public-key>" \
   zsh scripts/package-app.sh
   ```
   This produces `output/package/Session Hawk.dmg` and `output/package/Session Hawk.zip`.
3. **Sign the update zip with EdDSA** (for Sparkle auto-update):
   ...
   .build/artifacts/sparkle/Sparkle/Sparkle.xcframework/macos-arm64_x86_64/Sparkle.framework/Versions/B/bin/sign_update \
     "output/package/Session Hawk.zip"
   ```
...
6. **Create the release**:
   ...
   gh release create v<version> \
     "output/package/Session Hawk.dmg#Open.Island.dmg" \
     "output/package/Session Hawk.zip#Open.Island.zip" \
     --target main \
     --title "Session Hawk v<version> — <Title>" \
...
2. Since this is an unsigned app, macOS will show **"Session Hawk is damaged"** when you try to open it. Run this command in Terminal to fix it:
   由于应用未签名，macOS 会提示**「"Session Hawk"已损坏」**。请在终端中执行以下命令：
...
This stores the private key in your macOS Keychain and prints the public key. Save the public key — it goes into `SESSION_HAWK_EDDSA_PUBLIC_KEY` env var during packaging and into `SUPublicEDKey` in Info.plist.

## Signing (future)

When `SESSION_HAWK_SIGN_IDENTITY` is set, `package-app.sh` handles codesign + notarization automatically. At that point:
