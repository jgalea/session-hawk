...
`scripts/smoke-dev-app.sh` sets harness environment variables before launching `SessionHawkApp`.

The smoke path is intentionally aimed at the repository executable, not `~/Applications/Session Hawk Dev.app`. The dev bundle remains useful for manual end-to-end OSS verification, but harness automation should target the current branch's `SessionHawkApp` binary so the verification result matches the checked-out code exactly.

- `SESSION_HAWK_HARNESS_SCENARIO` selects a case from `IslandDebugScenario`
- `SESSION_HAWK_HARNESS_PRESENT_OVERLAY` mirrors the scenario onto the real island overlay
- `SESSION_HAWK_HARNESS_START_BRIDGE` skips live socket setup when disabled
- `SESSION_HAWK_HARNESS_BOOT_ANIMATION` disables the normal boot animation for deterministic runs
- `SESSION_HAWK_HARNESS_CAPTURE_DELAY_SECONDS` controls when artifact capture runs after launch
- `SESSION_HAWK_HARNESS_AUTO_EXIT_SECONDS` terminates the app automatically after the selected duration
- `SESSION_HAWK_HARNESS_ARTIFACT_DIR` selects the output directory for `report.json`, `timeline.json`, `runtime.log`, PNG captures, and `.ax.json` accessibility snapshots
