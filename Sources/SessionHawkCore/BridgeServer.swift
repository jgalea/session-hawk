...
public final class BridgeServer: @unchecked Sendable {
    ...
    private let queue = DispatchQueue(label: "com.jeangalea.sessionhawk.bridge.server")
    ...
    private func resolvePendingClaudeInteraction(
        ...
        case let (.permission(_), .deny(message, interrupt)):
            directive = .permissionRequest(
                .deny(message: message ?? "Permission denied in Session Hawk.", interrupt: interrupt)
            )
            summary = message ?? "Permission denied in Session Hawk."
            phase = .completed
