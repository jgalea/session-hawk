...
public enum HookSkipConfiguration {
    /// Preferred Session Hawk environment key for disabling hooks in this process.
    /// Session Hawk 推荐使用的当前进程 hook 跳过开关。
    public static let sessionHawkSkipKey = "SESSION_HAWK_SKIP_HOOKS"
    ...
    public static func shouldSkipHooks(environment: [String: String]) -> Bool {
        isTruthy(environment[sessionHawkSkipKey])
