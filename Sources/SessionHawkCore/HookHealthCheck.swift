...
/// Structured diagnostic result for a single hook integration (Claude Code).
public struct HookHealthReport: Equatable, Sendable {
    ...
    public var agent: String  // "claude"
        ...
        for (_, eventValue) in hooks {
            ...
            for group in groups {
                ...
                for hook in hookEntries {
                    ...
                    // Only check our own managed commands (current Session Hawk + legacy Open Island / Vibe Island)
                    let normalized = command.lowercased()
                    guard normalized.contains("sessionhawkhooks") || normalized.contains("session-hawk")
        ...
        for (_, eventValue) in hooks {
            ...
            for group in groups {
                ...
                for hook in hookEntries {
                    ...
                    // Skip our own hooks (current Session Hawk + legacy Open Island / Vibe Island)
                    if normalized.contains("sessionhawkhooks") || normalized.contains("session-hawk")
                        || normalized.contains("openislandhooks") || normalized.contains("vibeislandhooks")
