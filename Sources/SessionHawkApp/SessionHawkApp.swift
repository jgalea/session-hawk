...
@MainActor
final class SessionHawkAppDelegate: NSObject, NSApplicationDelegate {
    let model = AppModel()
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ProcessInfo.processInfo.disableAutomaticTermination(
            "Session Hawk should remain active while monitoring local agent sessions."
        )
        ...
        DispatchQueue.main.async { [self] in
            ...
            SessionHawkAppDelegate.hideAllAppWindows()
...
struct SessionHawkApp: App {
    @NSApplicationDelegateAdaptor(SessionHawkAppDelegate.self)
    private var appDelegate

    @Environment(\.openWindow) private var openWindow

    var body: some Scene {
        Window("Session Hawk Settings", id: "settings") {
