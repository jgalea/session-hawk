...
struct HarnessLaunchConfiguration {
    ...
    init(environment: [String: String] = ProcessInfo.processInfo.environment) {
        scenario = Self.scenarioValue(from: environment["SESSION_HAWK_HARNESS_SCENARIO"])
        presentOverlay = Self.boolValue(
            environment["SESSION_HAWK_HARNESS_PRESENT_OVERLAY"],
            default: false
        )
        shouldStartBridge = Self.boolValue(
            environment["SESSION_HAWK_HARNESS_START_BRIDGE"],
            default: true
        )
        shouldPerformBootAnimation = Self.boolValue(
            environment["SESSION_HAWK_HARNESS_BOOT_ANIMATION"],
            default: true
        )
        captureDelay = Self.timeIntervalValue(
            from: environment["SESSION_HAWK_HARNESS_CAPTURE_DELAY_SECONDS"]
        )
        autoExitAfter = Self.timeIntervalValue(
            from: environment["SESSION_HAWK_HARNESS_AUTO_EXIT_SECONDS"]
        )
        artifactDirectoryURL = Self.directoryURLValue(
            from: environment["SESSION_HAWK_HARNESS_ARTIFACT_DIR"]
        )
    }
