...
enum IslandDebugScenario: String, CaseIterable, Identifiable {
    ...
    func snapshot(at now: Date = .now) -> IslandDebugSnapshot {
        switch self {
        case .closed:
            let sessions = DebugSessionFixtures.listSessions(now: now)
        ...
        case .approvalCard:
            let session = DebugSessionFixtures.approvalSession(now: now)
            return IslandDebugSnapshot(
                title: title,
                summary: summary,
                previewHeight: 330,
                ...
                sessions: DebugSessionFixtures.notificationSessions(lead: session, now: now),
                selectedSessionID: session.id
            )

        case .questionCard:
            let session = DebugSessionFixtures.questionSession(now: now)
            return IslandDebugSnapshot(
                title: title,
                summary: summary,
                previewHeight: 270,
        ...
        case .completionCard:
            let session = DebugSessionFixtures.completionSession(now: now)
            return IslandDebugSnapshot(
                title: title,
                summary: summary,
                previewHeight: 250,
        ...
        case .longCompletionCard:
            let session = DebugSessionFixtures.longCompletionSession(now: now)
            return IslandDebugSnapshot(
                title: title,
                summary: summary,
                previewHeight: 290,
...
private enum DebugSessionFixtures {
    static func listSessions(now: Date) -> [AgentSession] {
        [
            ...
            inactiveSession(
                ...
                latestPrompt: "为什么要查 Claude 官方呢？这个事跟 Claude 有什么关系？",
            ...
            inactiveSession(
                id: "session-personal",
                workspace: "Personal",
                initialPrompt: "[Image #1]我给你截了 3 张图，这个是我现在 Claude 里面可用的模型。",
                latestPrompt: "[Image #1]我给你截了 3 张图，这个是我现在 Claude 里面可用的模型。",
            ...
            inactiveSession(
                ...
                assistant: "如果目标是轻量实时，不建议直接按 Claude 现成套餐来映射。",
                age: 78 * 60,
                now: now
            ),
            inactiveSession(
    ...
    static func runningSession(now: Date) -> AgentSession {
        AgentSession(
            ...
            jumpTarget: JumpTarget(
                ...
                paneTitle: "claude ~/Personal/session-hawk",
                workingDirectory: "/tmp/example/Personal/session-hawk",
            ...
            claudeMetadata: ClaudeSessionMetadata(
                ...
                currentToolInputPreview: "sed -n '1,260p' Sources/SessionHawkApp/Views/SettingsView.swift"
            )
        )
    }
    ...
    static func recentCompletedSession(now: Date) -> AgentSession {
        AgentSession(
            ...
            jumpTarget: JumpTarget(
                ...
                paneTitle: "claude ~/Personal/open-agent-sdk",
                workingDirectory: "/tmp/example/Personal/open-agent-sdk",
    ...
    static func inactiveSession(
        ...
        AgentSession(
            ...
            jumpTarget: JumpTarget(
                ...
                paneTitle: "claude ~/Personal/\(workspace)",
                workingDirectory: "/tmp/example/Personal/\(workspace)",
    ...
    static func approvalSession(now: Date) -> AgentSession {
        AgentSession(
            ...
            permissionRequest: PermissionRequest(
                ...
                summary: "Allow exec_command to rewrite SettingsView.swift?",
                affectedPath: "Sources/SessionHawkApp/Views/SettingsView.swift",
            ...
            claudeMetadata: ClaudeSessionMetadata(
                ...
                currentTool: "exec_command",
                currentToolInputPreview: "head -5000 /tmp/example/Personal/claude-research/extracts/claude-bun-2.1.81-v3/islands/000_cli.js.txt"
            )
        )
    }
...
[README.md](/tmp/example/Personal/session-hawk/README.md) 的现有改动已经单独提交了，commit 是 `f196316`，message 是 `docs: update readme tagline`。
