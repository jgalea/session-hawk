...
struct SettingsView: View {
    ...
    var body: some View {
        ...
        .onReceive(NotificationCenter.default.publisher(for: .sessionHawkSelectSetupTab)) { _ in
            selectedTab = .setup
        }
    }
...
struct SetupSettingsPane: View {
    ...
    private var allReady: Bool {
        model.claudeHooksInstalled && model.claudeUsageInstalled
    }
    ...
    private var hasErrors: Bool {
        (model.claudeHealthReport?.errors.count ?? 0) > 0
    }

    private var hasRepairableIssues: Bool {
        model.claudeHealthReport?.repairableIssues.isEmpty == false
    }

    private var hasNotices: Bool {
        model.claudeHealthReport?.notices.isEmpty == false
    }
    ...
    private var hookDiagnosticsSection: some View {
        Section {
            ...
            if model.claudeHealthReport == nil {
                HStack {
        ...
        Section {
            VStack(alignment: .leading, spacing: 12) {
                ...
                remoteSetupStep(
                    ...
                    description: "Run from the Session Hawk repo directory:",
